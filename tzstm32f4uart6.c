...
// 发送DMA
#define TX_DMA_INDEX 2
#define TX_DMA_CHANNEL 5
#define TX_DMA_STREAM 6

// 接收DMA
#define RX_DMA_INDEX 2
#define RX_DMA_CHANNEL 5
...
bool TZStm32f4Uart6Init(TZStm32f4UartParam param) {
    if (mid == -1) {
        mid = TZMallocRegister(0, TZSTM32F4UART6_TAG, param.MaxFrameSize * 2 + TZ_BUFFER_LEN);
        if (mid == -1) {
            return false;
        }
    }
