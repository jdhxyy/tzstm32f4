...
#define TX_DMA_STREAM 3

// 接收DMA
#define RX_DMA_INDEX 1
#define RX_DMA_CHANNEL 4
#define RX_DMA_STREAM 1

// 中断处理函数
#define TX_DMA_IRQHandler DMA1_Stream3_IRQHandler
...
bool TZStm32f4Uart3Init(TZStm32f4UartParam param) {
    if (mid == -1) {
        mid = TZMallocRegister(0, TZSTM32F4UART3_TAG, param.MaxFrameSize * 2 + TZ_BUFFER_LEN);
        if (mid == -1) {
            return false;
        }
        return false;
